...
def generate_strong_mapping(words):
    ...
    for word in words:
        while True:
            number = random.randint(1000, 9999)  # Generate a 4-digit number
...
pad = [str(random.randint(0, 9)) for _ in range(len(numeric_message.replace(" ", "")))]  # Length should match the numeric message without spaces
pad_str = ''.join(pad)
